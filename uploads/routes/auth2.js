// 구글 계정 연동 회원가입 및 로그인 
var express = require('express');
var router = express.Router();
var UserModel = require('../models/UserModel');
var passport = require('passport');
var GoogleStrategy = require('passport-google-oauth').OAuth2Strategy;

passport.serializeUser(function (user, done) {
    done(null, user);
});

passport.deserializeUser(function (user, done) {
    done(null, user);
});

passport.use(new GoogleStrategy({
        clientID: "884467909476-4dlme2rdnqa6tg40f3voqvhnnbe5jp3e.apps.googleusercontent.com", //앱 ID 입력하세요
        clientSecret: "1zo3FZTgjrQmZz_BpWkt3_58", // 앱 시크릿 코드 입력하세요.
        callbackURL: "http://localhost:3000/auth2/google/oauth2callback",
    },
    function(accessToken, refreshToken, profile, done) {
        console.log(profile);
        UserModel.findOne({ userid : "g_" + profile.id }, function(err, user){
            if(!user){  //없으면 회원가입 후 로그인 성공페이지 이동
                var regData = { //DB에 등록 및 세션에 등록될 데이터
                    userid :  "g_" + profile.id,
                    password : "google_login",
                    displayname : profile.displayName,
                    email_verification_state : true
                };
                var User = new UserModel(regData);
                User.save(function(err){ //DB저장
                    done(null,regData); //세션 등록
                });
            }else{ //있으면 DB에서 가져와서 세션등록
                done(null,user);
            }

        });
    }
));

// 구글 인증링크 생성
router.get('/google', passport.authenticate('google', { scope: ['email']}) );


//인증후 구글에서 이 주소로 리턴해줌. 상단에 적은 callbackURL과 일치
router.get('/google/oauth2callback',
    passport.authenticate('google', 
        { 
            // 구글 로그인 후 /로 리다이렉팅 하도록 설정
            successRedirect: '/',
            failureRedirect: '/auth2/google/fail' 
        }
    )
);

//로그인 성공시 이동할 주소
router.get('/google/success', function(req,res){
    res.send(req.user);
});

router.get('/google/fail', function(req,res){
    res.send('google login fail');
});

module.exports = router;